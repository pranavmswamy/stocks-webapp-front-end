...
export class TiingoService {
  ...
  getAutoCompleteOptions(text) {
    const autoCompleteUrl = `${this.hostUrlGCPServerless}autocomplete?query=${text}`
    return this.http.get<AutoCompleteElement[]>(autoCompleteUrl)
  }

  getCompanyDescription(ticker) {
    const companyDescriptionUrl = `${this.hostUrlGCPServerless}company-details?companyName=${ticker}`
  ...
  getDailyChartData(ticker, date) {
    ...
    const dailyChartDataUrl = `${this.hostUrlGCPServerless}daily-chart-data?companyName=${ticker}&startDate=${today}&resampleFreq=${resampleFreq}`
  ...
  getHistoricalData(ticker) {
    ...
    const historicalDataUrl = `${this.hostUrlGCPServerless}historical-data?companyName=${ticker}&startDate=${twoYearsAgo}&resampleFreq=${resampleFreq}`
    console.log('historical data- ', historicalDataUrl)
    return this.http.get(historicalDataUrl)
  }

  getLatestPrice(ticker) {
    const latestPriceUrl = `${this.hostUrlGCPServerless}latest-price?companyName=${ticker}`
