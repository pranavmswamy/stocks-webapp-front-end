...
export class TiingoService {
  ...
  getAutoCompleteOptions(text) {
    const autoCompleteUrl = `${this.urlLocal}autocomplete?query=${text}`
    return this.http.get<AutoCompleteElement[]>(autoCompleteUrl)
  }

  getCompanyDescription(ticker) {
    const companyDescriptionUrl = `${this.urlLocal}company-details?companyName=${ticker}`
  ...
  getDailyChartData(ticker, date) {
    ...
    const dailyChartDataUrl = `${this.urlLocal}daily-chart-data?companyName=${ticker}&startDate=${today}&resampleFreq=${resampleFreq}`
  ...
  getHistoricalData(ticker) {
    ...
    const historicalDataUrl = `${this.urlLocal}historical-data?companyName=${ticker}&startDate=${twoYearsAgo}&resampleFreq=${resampleFreq}`
    console.log('historical data- ', historicalDataUrl)
    return this.http.get(historicalDataUrl)
  }

  getLatestPrice(ticker) {
    const latestPriceUrl = `${this.urlLocal}latest-price?companyName=${ticker}`
